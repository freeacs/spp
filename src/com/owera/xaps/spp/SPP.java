...
public class SPP {
			...
			if (unit == null) {
				if (Properties.isDiscoveryMode()) {
					if (sessionData.getModelName() != null) {
						...
						Log.warn(SPP.class, "Serialnumber " + sessionData.getSerialNumber()
								+ " was not found in xAPS, even though discovery mode activiated, no modelname was found - so we cannot create unit");
						output = sessionData.getResp().getEmptyResponse();
					}
				} else {
	...
	private static Unit readUnit(SessionData sessionData) throws SQLException, NoAvailableConnectionException {
		...
		if (unit == null && sessionData.getSerialNumber() != null)
			unit = xapsUnit.getUnitByValue(sessionData.getSerialNumber(), null, null);
		if (unit == null && sessionData.getMac() != null)
			unit = xapsUnit.getUnitByValue(sessionData.getMac(), null, null);
		return unit;
	}
	...
	private static byte[] getResponseSoftwareUpgrade(SessionData sessionData, String dsw, Unit unit) throws SQLException {
		...
		} else { // Staging upgrade output - return binaries directly
	...
	private static void updateXAPS(SessionData sessionData, DBI dbi) throws NoAvailableConnectionException, SQLException {
		if (sessionData.getUnit() != null) {
			...
			//			addToUnitParameterList(SystemParameters.MAC, unitParameters, utps, sessionData.getMac(), unit, sessionData.getProfile());
			...
			// Missing parameters:
			// lastConnectMessage - will not have a purpose until we have the situation of a registered device
			// which is not possible to provision (like failed authentication - which is currently not implemented)
